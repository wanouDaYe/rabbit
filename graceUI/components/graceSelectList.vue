		...
		<view v-for="(item, index) in dataIn" :key="index" class="select-list-item" :data-index="index" @tap.stop="choose">
			...
			<view class="select-list-body" :class="[isBorder?'grace-border-b':'']" :style="{borderColor:borderColor}">
				<view class="select-list-content">
					<text :style="{fontSize:fontSize, lineHeight:lineHeight, color:itemColor, display:'block', width:'100%'}">{{item.title}}</text>
...
export default{
	props:{
		...
		checkColor:{type:String, default:"#3B68EA"},
...
.select-list-desc{font-size:22rpx; color:#828282; line-height:40rpx; display:block; width:100%;}
</style>
